# 目标网址 airbnb爱彼迎民宿短租平台爬虫（tls指纹） https://zh.airbnb.com/
# 抓取大阪市下所有的房源信息
...
def spider_airbnb():
            ...
            "staysSearchRequest": {
                ...
                "rawParams": [
                    ...
                    },
                    {
                        ...
                        "filterValues": [
                            "24cd930d-5730-409d-9456-c20e3be0adbc"
                        ]
                    },
                    {
    ...
    # b9389bf17721a08edbea5073307d377d39325b2e4acaf008e87d847a2e112cf8为sha256的固定hash值，所有请求大阪市下的内容都是同一个地址
    ...
    while True:
        try:
            ...
            resp = requests.post(url, headers=headers, json=data,impersonate='chrome120',timeout=15)
            ...
            # 一行代码，全部抽离
            clean_listings = jmespath.search(data_expression, resp_data)
            ...
            # 追加进all_room_data中
            all_room_data.extend(clean_listings)

            # 下一页数据的cursor值
            next_cursor_expression =f'data.presentation.staysSearch.results.paginationInfo.pageCursors[{page}]'
            next_cursor_page = jmespath.search(next_cursor_expression, resp_data)
            ...
            # 替换现有的data载荷里的cursor
            data['variables']['staysSearchRequest']['cursor'] = next_cursor_page
            page += 1
...
# 进一步优化点：
"""
1.避免循环追加进数组，再最后导入到excel中，如果反爬了数据没有保存白忙活
解决方法：每爬完一页并保存到数据库或csv/excel中
2.使用session自动更新cookie值
"""
